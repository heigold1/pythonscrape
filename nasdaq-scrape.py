...
print("</head>")
print("<body>")
print("<h2>Hello Word  test! This is my first Python Script run through Apache</h2>")
print("</body>")
print("</html>")

def parse_finance_page(ticker):
  print('inside ddd parse_finance_page<br>') 
  ...
  print('just created the headers ' + ticker) 

  for retries in range(5):
    try:    
#      url = "https://www.nasdaq.com/symbol/%s"%(ticker)

      url = "https://www.sec.gov/cgi-bin/browse-edgar?CIK=MSFT&owner=exclude&action=getcompany"
...
scraped_data = parse_finance_page('MSFT')
